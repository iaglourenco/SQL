...
--CODIGOS EM PL-SQL PARA MANIPULAÇÃO DO BANCO DE DADOS
...
BEGIN
	...
	FOR k IN (SELECT COD_DISC FROM GRADE WHERE NUM_CURSO = (SELECT NUM_CURSO FROM ALUNOS WHERE RA=REGISTRO) MINUS SELECT COD_DISC FROM CURSARAM WHERE RA=REGISTRO AND STATUS = 'A' MINUS SELECT COD_DISC FROM CURSAM WHERE RA=REGISTRO))
	LOOP
...
CREATE OR REPLACE FUNCTION VERIFICA(REGISTRO NUMBER,CODIGO NUMBER) RETURN NUMBER IS
VALUER NUMBER;
BEGIN
...
LOOP
	...
	IF I.COD_DISC = CODIGO THEN
		...
		--COMPARAR COM CURSADAS SE PREENCHE TODOS OS PREREQ
...
DBMS_OUTPUT.PUT_LINE('ALUNO NAO APTO');
RETURN 0;
END;
/

--VERIFICAR SE O ALUNO ESTA APTO A CURSAR A DISCIPLINA USANDO A FUNCTION VERIFICA
CREATE OR REPLACE TRIGGER VERIFICA_APTIDAO BEFORE INSERT IN CURSAM
FOR EACH ROW
VALUER NUMBER;
BEGIN 

	SELECT VERIFICA(:NEW.RA,:NEW.COD_DISC) INTO VALUE FROM DUAL;
